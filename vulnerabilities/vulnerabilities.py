...
def update_assignments(
    ...
    #if source.label.count("url") > 0:
        #print("propagation source is: " + str(source))
    for node in assignment_nodes:
        #if source.label.count("url") > 0:
            #print(str(node.__repr__()))
        #print("Assignment list is:")
...
def append_node_if_reassigned(
    ...
    # print("append node if reassigned")
    # print("secondary left hand side: " + str(secondary.left_hand_side))
    # print("right hand side: " + str(node.right_hand_side_variables))
    if (
        secondary.left_hand_side in node.right_hand_side_variables or
        secondary.left_hand_side == node.left_hand_side
    ):
        #print("appended: " + str(node))
...
def get_vulnerability_chains(
    ...
    for use in def_use[current_node]:
        ...
        #     print("error in vulnerability section")
...
def get_vulnerability(
    ...
    if tainted_node_in_sink_arg:
        if sink.trigger_word.startswith("xbmc.executebuiltin") and source.trigger_word.startswith("Framework function URL parameter"):
            print("Found tainted node in sink args")
        vuln_deets = {
...
def find_vulnerabilities_in_cfg(
    ...
    print("all sources: " + str(triggers.sources) + "\n\nall sinks: " + str(triggers.sinks))
