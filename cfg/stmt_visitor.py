...
from cfg.stmt_visitor_helper import (
    CALL_IDENTIFIER,
    ConnectStatements,
    connect_nodes,
...
class StmtVisitor(ast.NodeVisitor):
    ...
    def add_blackbox_or_builtin_call(self, node, blackbox):  # noqa: C901
        ...
        for arg_node in itertools.chain(node.args, node.keywords):
            ...
            arg_collector = RHSVisitor()
            arg_collector.visit(arg_node)
            # print("args found: " + str(arg_collector.result))
            if isinstance(arg, ast.Call):
                ...
                if isinstance(arg_node, ast.keyword) and arg_node.arg is not None:
                    ...
                    visual_args.extend(arg_collector.result)
                rhs_vars.append(return_value_of_nested_call.left_hand_side)
                rhs_vars.extend(arg_collector.result)
            else:
                label = LabelVisitor()
    ...
    def visit_With(self, node):
        ...
        for item in node.items:
            if isinstance(item.context_expr, ast.Call) and isinstance(item.context_expr.func, ast.Name) and item.context_expr.func.id == "open" and isinstance(item.context_expr.args[0], ast.Name):
                target_node = _ast.Name(id=item.context_expr.args[0].id, ctx=ast.Store())
                value_node = _ast.Name(item.optional_vars.id, ctx=ast.Load())
                # target_node.lineno = node.lineno
                # value_node.lineno = node.lineno
                node.body.append(_ast.Assign(targets=[target_node], value=value_node))
