...
from cfg.alias_helper import handle_aliases_in_calls
...
from cfg.expr_visitor_helper import (
    BUILTINS,
    MUTATORS,
...
from cfg.stmt_visitor import StmtVisitor
from cfg.stmt_visitor_helper import CALL_IDENTIFIER
...
class ExprVisitor(StmtVisitor):
    ...
    def visit_With(self, node):
        for body_statement in node.body:
            self.append_node(self.visit(body_statement))
        for item in node.items:
            return self.visit_withitem(item, node)
    ...
    def visit_withitem(self, node, parent):
        if isinstance(node.context_expr, ast.Call) and isinstance(node.context_expr.func, ast.Name) and node.context_expr.func.id == "open":
            label = LabelVisitor()
            label.visit(node)
            return self.append_node(AssignmentNode(label.result, node.context_expr.args[0].id, parent, [node.optional_vars.id], path=self.filenames[-1]))
    '''
