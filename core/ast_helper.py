...
def _convert_to_3(path):  # pragma: no cover
    ...
    try:
        ...
        # print(subprocess.check_output(['2to3', '-w', path]))
...
def fix_indents(path):
    try:
        print("Tried to fix indents in: " + path)
        subprocess.call(['reindent', '-n', path])
        # indenter = reindent.Reindenter(path)
        # indenter.run()
    except subprocess.SubprocessError:
        print("Error fixing indents: " + path)
...
def _list_to_dotted_string(list_of_components):
    """Convert a list to a string separated by a dot."""
